    ...
    def planning(self, sx, sy, gx, gy):
        ...
        while True:
            ...
            if current.x == goal_node.x and current.y == goal_node.y:
                ...
                goal_node.cost = current.cost * 2  # Double the final cost
                break
...
def main():
    ...
    T_CONST = 76.15432893255064  # fixed time value, user will not input t

    # initialize r so it's always defined
    r = None

    # ask user for fuel price f (must be provided before other inputs)
    try:
        f = float(input("Enter fuel price (f): "))
    except Exception:
        print("Invalid fuel price. Using default f=0.8")
        f = 0.8

    # --- ask user for number of passengers and max flights, then select aircraft ---
    try:
        n = int(input("Enter number of passengers (n): "))
        Max = int(input("Enter Max number of flights (Max): "))
        if Max == 0:
            print("Max cannot be zero. Exiting.")
            return
        r = n / Max
        print(f"Passengers per flight (r) = {r}")
        ...
        print(f"Aircraft selected based on passengers -> {ac}")

        # cost = 0.8*a1*t + a2*t + a3
        if ac in ("A320", "A321"):
            a1 = 54.0
            a3 = 1800.0
            c = input("Enter time-cost category (L/M/H): ").strip().upper()
            a2_map = {"L": 10.0, "M": 15.0, "H": 20.0}
            a2 = a2_map.get(c, 10.0)
            print(f"{ac} selected -> a1={a1}, a2={a2}, a3={a3}")

        elif ac == "A339":
            a1 = 84.0
            a3 = 2000.0
            c = input("Enter time-cost category (L/M/H): ").strip().upper()
            a2_map = {"L": 15.0, "M": 21.0, "H": 27.0}
            a2 = a2_map.get(c, 15.0)
            print(f"{ac} selected -> a1={a1}, a2={a2}, a3={a3}")
            ...
            a3 = 2500.0
            c = input("Enter time-cost category (L/M/H): ").strip().upper()
            a2_map = {"L": 20.0, "M": 27.0, "H": 34.0}
            a2 = a2_map.get(c, 20.0)
            print(f"{ac} selected -> a1={a1}, a2={a2}, a3={a3}")

        else:
            # fall back to manual input if needed (user will not input t)
            a1 = float(input("Enter aircraft type coefficient (a1): "))
            a2 = float(input("Enter operation cost coefficient (a2): "))
            a3 = float(input("Enter fixed cost (a3): "))

        # use fixed time constant
        t = T_CONST
        print(f"Using fixed time t = {t}")

    except Exception:
        print("Invalid input. Using defaults a1=1.0, a2=10.0, a3=1800.0, t=T_CONST")
        a1, a2, a3, t = 1.0, 10.0, 1800.0, T_CONST

    # show passengers per flight result r if available
    if r is not None:
        print(f"Passengers per flight (r) = {r}")
    else:
        print("Passengers per flight (r) not available")

    # replace the hard-coded 0.8 with user-provided fuel price f
    total_cost = f * a1 * t + a2 * t + a3
    print("Computed total cost ->", total_cost)

    # display total cost multiplied by maximum flight number (Max)
    try:
        total_cost_all = total_cost * Max
        print(f"Total cost for {Max} flights -> {total_cost_all}")
    except NameError:
        print("Max not available; cannot compute total cost for Max flights")
    ...
    for i in range(30, 40):
        for j in range(0, 20):
            fc_x.append(i)
            fc_y.append(j)
            # Second region - adding another cost intensive area 2
